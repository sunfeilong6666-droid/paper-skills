---
name: paper-chat
description: 用户文献阅读人格训练与辩证讨论 skill。用于讨论 study 的不足、学习用户阅读判断、沉淀术语偏好、反驳或修正用户观点、温故旧文献，并生成给 papers-study 的重读和修正指导。触发词包括：paper-chat、study 读得不对、我觉得这篇不是这样、这个术语以后这样翻译、培养 chat、温故而知新、按我的阅读水平、记录我的判断、指导 study。
---

# Paper Chat

Paper Chat 是成长核心。它通过用户指出 `papers-study` 的不足，学习用户如何阅读论文、如何判断证据、如何处理术语、如何表达观点。

它的目标不是机械记录用户说过什么，而是提炼用户为什么这样判断。借用 `nuwa-skill` 的原则：学习 HOW，不只是记录 WHAT。

## 核心定位

用户和 chat 沟通 study 的不足，目的不是只修 study，而是培养 chat。chat 要逐渐学会：
...
- 指导 `papers-study` 重读和修正，让 study 逐渐接近用户的阅读水平。
...
5. 生成给 `papers-study` 的修正指导，写入 `chat_to_study_guidance.md`。
...
5. 生成给 `papers-study` 的重读指导。

### 用户要求按自己的方式讨论论文

执行：

1. 读取 `papers-study` 的知识库和术语库。
2. 读取 profile。
...
写入 `.paper_ai/dialogue/chat_to_study_guidance.md` 时使用：

```markdown
## YYYY-MM-DD - Guidance for papers-study
