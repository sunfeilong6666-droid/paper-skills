...
本模块是不可被记录格式偏好覆盖的人格学习与证据边界。不同模型即使有主动分派子 agent 的习惯，也必须优先遵守本模块。

- 主 agent 必须亲自完成用户反馈理解、阅读规则提炼、证据边界判断、profile 更新和 `memory_to_study_guidance.md` 写入。
- 主 agent 必须亲自区分原文事实、作者声称、study 判断、用户理解和待验证假设。
- 子 agent 不得作为用户阅读人格学习者、观点裁判、profile 作者或给 study 的指导作者使用。
- 子 agent 只能做低风险辅助：定位相关文件位置、摘录待主 agent 判断的上下文、检查记录格式是否完整、检查是否遗漏必填字段、对主 agent 已写好的内容做忠实中文润色。
- 子 agent 的输出不能直接成为长期 profile 规则；主 agent 必须复核并判断是临时规则还是稳定规则。

## 可变模块：用户可调整的记录格式

本模块可以随用户偏好调整，但不得改变上面的主子 agent 边界和证据优先原则。

- 用户可以增减 `study_feedback.md` 的字段，例如增加“用户原话”“反例”“后续验证计划”。
- 用户可以增减 `memory_to_study_guidance.md` 的字段，例如增加“图表重读要求”“术语禁用译法”“写作可用程度”。
- 用户可以调整 profile 文件的细分方式或表达风格。
- 格式变化不能把一次性观点自动升级为长期规则，不能把用户偏好写成文献事实。
...
- 不要替代 
ature-writing 写正式论文。
- 不要调用子 agent 学习用户阅读人格、裁判用户观点或生成正式 profile。
- 不要在没有文献证据时强行反驳用户。
- 不要把用户表达风格凌驾于事实准确性之上。
