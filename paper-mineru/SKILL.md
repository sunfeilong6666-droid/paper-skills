...
本模块是不可被记录格式偏好覆盖的工程边界。

- 主 agent 必须亲自确认项目根目录、PDF 来源、规范化输入目录、MinerU 输出目录、配置文件、命令风险和是否获得运行授权。
- 子 agent 可以承担低风险工程辅助，例如扫描目录、清点 PDF、整理映射表候选、检查命令文本、汇总运行日志。
- 子 agent 不得总结论文、判断论文贡献、建立知识库、更新术语库或判断用户阅读习惯。
- 子 agent 不得自行运行 MinerU；只有用户明确授权运行且主 agent 已审核命令后，才能按当前环境允许的方式启动或监控任务。
- 子 agent 的输出不能直接改写关键配置或状态；主 agent 必须复核路径、命令和文件映射后再写入。
...
默认生成高质量解析命令。MinerU 命令中的 `-p` 和 `-o` 必须使用上面解析出的绝对路径，不要使用相对 `.paper_ai/...`。不要运行 MinerU，除非用户明确说“运行 MinerU”“开始解析”“run MinerU”。
...
- 不要让子 agent 绕过主 agent 审核执行路径判断、状态写回或命令运行。
