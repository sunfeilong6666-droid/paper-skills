...
从`.paper_ai/knowledge/01mineru/mineru_wait.txt`中读取所有 PDF的路径，将对应文件复制到：
...
```text
.paper_ai/knowledge/00test/paper_file_map.md
```
...
no_proxy=localhost,127.0.0.1,::1
```
读取存放项目根目录的`.paper_ai/knowledge/00test/work.txt`文件


在桌面打开终端，**必须是长时间显示的窗口**。
禁止使用powershell
将最终命令写入 `.paper_ai/knowledge/00test/mineru_run.txt`
**逐行**读取`.paper_ai/knowledge/00test/mineru_run.txt`，**逐行**向终端输入对应的代码，
...
export no_proxy="localhost,127.0.0.1,::1"
conda activate <ENV_NAME>
mineru -p "<项目根目录>\paper_ai\knowledge\00papers" -o "<项目根目录>\.paper_ai\knowledge\01mineru" -m auto -b vlm-engine --image-analysis truee
```
